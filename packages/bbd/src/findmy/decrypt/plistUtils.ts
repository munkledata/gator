...
/**
 * Plist helpers shared by the Find My cache decryptors. Ported from upstream PR #810.
 */
...
export const parsePlistFile = async (filePath: string): Promise<any> => {
    return parsePlistBuffer(fs.readFileSync(filePath));
};
 ...
 *
 * The keychain bplist nests the raw key as: symmetricKey -> key -> data. Older/alternate formats
 * may store the key directly as a base64 string.
 *
