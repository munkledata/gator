...
/**
 * Decryption for Apple Find My's encrypted `LocalStorage.db` (friend coordinates), macOS 14.0+.
 * Ported from upstream PR #810 (itself ported from findmy-key-extractor/decrypt_localstorage.py).
 *
 * This is NOT standard SQLCipher. Apple's `sqliteCodecCCCrypto` encrypts each 4096-byte SQLite page
 * independently using an AES-256 keystream-XOR construction:
...
const decryptPage = (key: Buffer, page: Buffer, pageIndex: number): Buffer => {
    ...
    for (let i = 0; i < CONTENT_LEN; i++) {
        // i < CONTENT_LEN < PAGE_SIZE and both buffers are PAGE_SIZE — indices are always present.
