...
export class MessageSender {
    ...
    async sendReaction(input: SendReactionInput): Promise<SendResult> {
        if (!this.#transport.isConnected()) {
            throw new Error("reactions require the Private API (helper not connected)");
        }
        const res = await this.#transport.send({
            action: "send-reaction",
            data: { chatGuid: input.chatGuid, messageGuid: input.messageGuid, reactionType: input.reactionType }
        });
        if (res.error) throw new Error(res.error);
        return { guid: res.identifier, viaPrivateApi: true };
    }
